...
/*   Updated: 2023/03/18 20:36:33 by evaluation       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_a(t_stack_a *stack_a)
{
	...
	if (stack_a->current_size < 2)
		return ;
	tmp = *stack_a->top;
	memmove(stack_a->bottom + 1, stack_a->bottom,
...
void	rotate_b(t_stack_b *stack_b)
{
	...
	if (stack_b->current_size < 2)
		return ;
	tmp = *stack_b->top;
	memmove(stack_b->bottom + 1, stack_b->bottom,
...
void	reverse_rotate_a(t_stack_a *stack_a)
{
	...
	if (stack_a->current_size < 2)
		return ;
	tmp = *stack_a->bottom;
	memmove(stack_a->bottom, stack_a->bottom + 1,
		(size_t)(stack_a->current_size - 1) * sizeof (int));
	*stack_a->top = tmp;
}
...
void	reverse_rotate_b(t_stack_b *stack_b)
{
	...
	if (stack_b->current_size < 2)
		return ;
	tmp = *stack_b->bottom;
	memmove(stack_b->bottom, stack_b->bottom + 1,
		(size_t)(stack_b->current_size - 1) * sizeof (int));
	*stack_b->top = tmp;
}
